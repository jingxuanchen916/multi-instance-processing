...
class NewsDataset(BaseDataset):
    """
    Dataset for news category classification evaluation tasks.
    Combines news article text data with category labels and news-specific questions.
    Labels: 0 = business, 1 = entertainment, 2 = politics, 3 = sport, 4 = tech
    """
    
    # Standard progression of article counts for testing (news only goes up to 200)
    STANDARD_SIZES = [1, 2, 5, 10, 20, 50, 100, 200]
    ...
    def __init__(self, data_dir: str, augment_approach: str):
        ...
        Args:
            data_dir: Directory containing news files (text, labels, questions.yaml)
            augment_approach: Augmentation approach ('default', 'head', 'middle', 'tail', 'random')
        """
        super().__init__(data_dir, augment_approach)
        
        # Define file paths
        self.text_file = self.data_dir / "250_news_text.txt"
        self.labels_file = self.data_dir / "250_news_label.txt"
        # Questions file is now in config/questions/
        self.questions_file = self.data_dir.parent.parent / "config" / "questions" / "news_question.yaml"
    ...
    def load_data(self) -> List[str]:
        """
        Load news article text data from file.
        
        Returns:
            List of news article strings
        """
        ...
        logger.info(f"Loaded {len(articles)} news articles from {self.text_file}")
        return articles
    
    def format_for_prompt(self, data_slice: List[str]) -> str:
        """
        Format news articles for inclusion in LLM prompt.
        Each article is numbered for clarity, with optional noise injection based on augmentation approach.
        
        Args:
            data_slice: List of news article strings
