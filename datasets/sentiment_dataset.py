"""Dataset implementation for sentiment analysis evaluation tasks."""
...
class SentimentDataset(BaseDataset):
    """
    Dataset for sentiment analysis evaluation tasks.
    Combines movie review text data with sentiment labels and sentiment-specific questions.
    Labels: 0 = negative, 1 = positive
    """
    
    # Standard progression of review counts for testing
    ...
    def __init__(self, data_dir: str, augment_approach: str):
        ...
        Args:
            data_dir: Directory containing sentiment files (text, labels, questions.yaml)
            augment_approach: Augmentation approach ('default', 'head', 'middle', 'tail', 'random')
        """
        super().__init__(data_dir, augment_approach)
        
        # Define file paths
        self.text_file = self.data_dir / "2500_sentiment_text.txt"
        self.labels_file = self.data_dir / "2500_sentiment_label.txt"
        # Questions file is now in config/questions/
        self.questions_file = self.data_dir.parent.parent / "config" / "questions" / "sentiment_question.yaml"
    ...
    def load_data(self) -> List[str]:
        """
        Load review text data from file.
        ...
        logger.info(f"Loaded {len(reviews)} reviews from {self.text_file}")
        return reviews
    
    def format_for_prompt(self, data_slice: List[str]) -> str:
        """
        Format reviews for inclusion in LLM prompt.
        Each review is numbered for clarity, with optional noise injection based on augmentation approach.
        
        Args:
            data_slice: List of review strings
